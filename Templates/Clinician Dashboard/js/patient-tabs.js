...
function renderOverview(id){
  ...
  const vitCards = vitMeta.map(m => {
    const v = d.vitals[m.key];
    return `<div class="vc">
      <div class="vn">${m.label} <span class="vsrc">${m.src}</span></div>
      <div><span class="vval">${v.val}</span>${v.unit?`<span class="vunit">${v.unit}</span>`:''}</div>
      <div class="vtrend ${v.tc}">${v.trend}</div>
      ${mkBars(v.bars,v.tc)}
    </div>`;
  }).join('');
  ...
  return `<div class="ogrid">
    <div class="twin-card">
      ...
      <div class="twin-img-wrap"><img class="twin-img" src="twin.png" alt="Digital Twin"/></div>
      <div class="score-row">
